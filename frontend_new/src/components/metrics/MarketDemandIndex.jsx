...
export default function MarketDemandIndex() {
  ...
  useEffect(() => {
    if (!selected) return;
    setLoading(true);
    apiClient.get(`/agent/metrics/market-demand-index?product_name=${encodeURIComponent(selected)}`)
    ...
    <div>
      ...
      <ResponsiveContainer width="100%" height={260}>
        <LineChart data={data || []} margin={{ top: 5, right: 20, bottom: 5, left: 5 }}>
          ...
          <YAxis tick={{ fontSize: 11 }} domain={[50, 130]} label={{ value: 'Demand Index', angle: -90, position: 'insideLeft', style: { fontSize: 10 } }} />
